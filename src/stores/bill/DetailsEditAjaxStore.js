 ...
 * api:http://git.yonyou.com/sscplatform/fc_doc/blob/master/expense-web-reimburse.md
 */
...
export default class DetailsAjaxStore{
  ...
  @observable DetailsTableColumn = {
    travel:[//出行
      ...
      {type: 'string', id: '_reductionAmount', label: (<span>核报金额<span className="glyphicon glyphicon-pencil"></span></span>), columnClassName:'table-edit-columns'},
  ...
  */
        ...
        if('infoType' in item && item.infoType=='travel'){
          if(item.infoList && item.infoList.length>0){
            item.infoList.forEach((zItem,zIndex)=>{
              ...
              zItem._reductionAmount = zItem.reduction_amount>=0 ? that.formatCurrency(zItem.reduction_amount) : that.formatCurrency(zItem.money)
              zItem.reductionInfo = zItem.reduction_info ? zItem.reduction_info : ''
            })
          }
  ...
  getCShareList(data){
    ...
    if(data.expenseCShareList && data.expenseCShareList.length>0){
      ...
      data.expenseCShareList.forEach((item,index)=>{
        item.pk = item.cshare_pk;
        item.assume_amount
        item._reductionAmount=item._reductionAmount?item._reductionAmount : that.formatCurrency(item.assume_amount)
        item.reductionInfo = item.reduction_info ? item.reduction_info : ''
  ...
  getSubsidy(data){
    ...
    if(data.subsidy!=null){
      ...
      subsidy.pk = data.subsidy.subsidy_pk
      subsidy.days = data.subsidy.days?data.subsidy.days:data.subsidy.subsidyDays
      subsidy.units = data.subsidy.units?data.subsidy.units:data.subsidy.subsidyUnits
      subsidy._reductionAmount='_reductionAmount' in data.subsidy ? that.formatCurrency(data.subsidy._reductionAmount):that.formatCurrency(subsidy.subsidyAmount)
      subsidy.reductionInfo = data.subsidy.reduction_info ? data.subsidy.reduction_info : ''
      list.push(subsidy);
    }
...
@observable approveData = {}
  ...
  */
  @action queryBillDetailsData(data,callback){
    var that =this;
    ...
    $.ajax({
        ...
        success: data => {
          ...
          if (data.code==0) {
            ...
            // console.log(that.headerListData);
